...
class LoginPage(BasePage):
    ...
    def on_building_ui(self):
        gr.Markdown(f"# Chào mừng đến với ứng dụng {self._app.app_name}!")
        self.usn = gr.Textbox(label="Tên đăng nhập", visible=False)
        self.pwd = gr.Textbox(label="Mật khẩu", type="password", visible=False)
        self.btn_login = gr.Button("Đăng nhập", visible=False)
    ...
    def login(self, usn, pwd, request: gr.Request):
        ...
        if user:
            ...
            with Session(engine) as session:
                ...
                gr.Warning("Tên đăng nhập hoặc mật khẩu không hợp lệ")
                return None, usn, pwd
