...
class HelpPage:
    def __init__(
        self,
        app,
        doc_dir: str = settings.KH_DOC_DIR / ".." / "libs" / "ktem" / "ktem" / "pages" / "help-md",
        ...
        about_md_dir = self.doc_dir / "about-vn.md"
        if about_md_dir.exists():
            with (self.doc_dir / "about-vn.md").open(encoding="utf-8") as fi:
                about_md = fi.read()
        else:  # fetch from remote
            about_md = get_remote_doc(
                f"{self.remote_content_url}/v{self.app_version}/docs/about-vn.md"
            )
        if about_md:
            with gr.Accordion("Giới thiệu"):
                if self.app_version:
                    about_md = f"Phiên bản: {self.app_version}\n\n{about_md}"
                gr.Markdown(about_md)

        if KH_DEMO_MODE:
            with gr.Accordion("Tạo Không gian Riêng của Bạn"):
                gr.Markdown(
                    "Đây là bản demo với chức năng hạn chế. "
                    "Sử dụng nút **Tạo không gian** để cài đặt Kotaemon "
                    "trong không gian riêng của bạn với đầy đủ tính năng "
                    "(bao gồm tải lên và quản lý tài liệu riêng tư "
                    "một cách an toàn)."
                )
                gr.Button(
                    value="Tạo Không gian Riêng của Bạn",
                    link=HF_SPACE_URL,
                    variant="primary",
                    size="lg",
                )

        user_guide_md_dir = self.doc_dir / "usage-vn.md"
        if user_guide_md_dir.exists():
            with (self.doc_dir / "usage-vn.md").open(encoding="utf-8") as fi:
                user_guide_md = fi.read()
        else:  # fetch from remote
            user_guide_md = get_remote_doc(
                f"{self.remote_content_url}/v{self.app_version}/docs/usage-vn.md"
            )
        if user_guide_md:
            with gr.Accordion("Hướng dẫn Sử dụng", open=not KH_DEMO_MODE):
        ...
        if self.app_version:
            ...
            if changelogs:
                with gr.Accordion(f"Nhật ký Thay đổi (v{self.app_version})"):
