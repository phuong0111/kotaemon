...
USERNAME_RULE = """**Quy tắc tên đăng nhập:**

- Tên đăng nhập không phân biệt chữ hoa chữ thường
- Tên đăng nhập phải có ít nhất 3 ký tự
- Tên đăng nhập không được quá 32 ký tự
- Tên đăng nhập chỉ được chứa các ký tự chữ và số và dấu gạch dưới
"""


PASSWORD_RULE = """**Quy tắc mật khẩu:**

- Mật khẩu phải có ít nhất 8 ký tự
- Mật khẩu phải chứa ít nhất một chữ cái viết hoa
- Mật khẩu phải chứa ít nhất một chữ cái viết thường
- Mật khẩu phải chứa ít nhất một chữ số
- Mật khẩu phải chứa ít nhất một ký tự đặc biệt từ danh sách sau:
    ^ $ * . [ ] { } ( ) ? - " ! @ # % & / \\ , > < ' : ; | _ ~  + =
"""
...
def validate_username(usn):
    ...
    if len(usn) > 32:
        errors.append("Tên đăng nhập không được quá 32 ký tự")
...
class UserManagement(BasePage):
    def __init__(self, app):
        ...
        if hasattr(flowsettings, "KH_FEATURE_USER_MANAGEMENT_ADMIN") and hasattr(
            ...
            if is_created:
                gr.Info(f'Người dùng "{usn}" đã được tạo thành công')

    def on_building_ui(self):
        with gr.Tab(label="Danh sách người dùng"):
            self.state_user_list = gr.State(value=None)
            self.user_list = gr.DataFrame(
                headers=["id", "Tên người dùng", "Quản trị"],
            ...
            with gr.Group(visible=False) as self._selected_panel:
                ...
                self.usn_edit = gr.Textbox(label="Tên đăng nhập")
                with gr.Row():
                    self.pwd_edit = gr.Textbox(label="Thay đổi mật khẩu", type="password")
                    self.pwd_cnf_edit = gr.Textbox(
                        label="Xác nhận thay đổi mật khẩu",
                        type="password",
                    )
                self.admin_edit = gr.Checkbox(label="Quản trị viên")

            with gr.Row(visible=False) as self._selected_panel_btn:
                with gr.Column():
                    self.btn_edit_save = gr.Button("Lưu")
                with gr.Column():
                    self.btn_delete = gr.Button("Xóa")
                    with gr.Row():
                        self.btn_delete_yes = gr.Button(
                            "Xác nhận xóa", variant="primary", visible=False
                        )
                        self.btn_delete_no = gr.Button("Hủy", visible=False)
                with gr.Column():
                    self.btn_close = gr.Button("Đóng")

        with gr.Tab(label="Tạo người dùng"):
            self.usn_new = gr.Textbox(label="Tên đăng nhập", interactive=True)
            self.pwd_new = gr.Textbox(
                label="Mật khẩu", type="password", interactive=True
            )
            self.pwd_cnf_new = gr.Textbox(
                label="Xác nhận mật khẩu", type="password", interactive=True
            )
            with gr.Row():
                gr.Markdown(USERNAME_RULE)
                gr.Markdown(PASSWORD_RULE)
            self.btn_new = gr.Button("Tạo người dùng")
    ...
    def create_user(self, usn, pwd, pwd_cnf):
        ...
        with Session(engine) as session:
            ...
            result = session.exec(statement).all()
            if result:
                gr.Warning(f'Tên đăng nhập "{usn}" đã tồn tại')
                return
    ...
    def list_users(self, user_id):
        ...
        with Session(engine) as session:
            ...
            results = [
                {"id": user.id, "Tên đăng nhập": user.username, "Quản trị": "Có" if user.admin else "Không"}
    ...
    def select_user(self, user_list, ev: gr.SelectData):
        if ev.value == "-" and ev.index[0] == 0:
            gr.Info("Không có người dùng nào được tải. Vui lòng làm mới danh sách người dùng")
            return -1
    ...
    def on_btn_delete_click(self, selected_user_id):
        if selected_user_id is None:
            gr.Warning("Chưa chọn người dùng nào")
            btn_delete = gr.update(visible=True)
            btn_delete_yes = gr.update(visible=False)
            btn_delete_no = gr.update(visible=False)
            return
    ...
    def save_user(self, selected_user_id, usn, pwd, pwd_cnf, admin):
        ...
        with Session(engine) as session:
            ...
            session.commit()
            gr.Info(f'Người dùng "{usn}" đã được cập nhật thành công')

        return "", ""

    def delete_user(self, current_user, selected_user_id):
        if current_user == selected_user_id:
            gr.Warning("Bạn không thể xóa chính mình")
        ...
        with Session(engine) as session:
            ...
            session.delete(user)
            session.commit()
            gr.Info(f'Người dùng "{user.username}" đã được xóa thành công')
        return -1
