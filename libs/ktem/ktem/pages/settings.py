    ...
    **IMPORTANT**: the name and id of the UI setting components should match the
    ...
    def on_building_ui(self):
        ...
        if self._app.f_user_management:
            with gr.Tab("Cài đặt người dùng"):
                self.user_tab()
    ...
    def on_register_events(self):
        ...
        if self._app.f_user_management and not KH_SSO_ENABLED:
            ...
            onSignOutClick = self.signout.click(
                lambda: (None, "Người dùng hiện tại: ___", "", ""),
                inputs=[],
                outputs=[
                    self._user_id,
                    self.current_name,
    ...
    def user_tab(self):
        # user management
        self.current_name = gr.Markdown("Người dùng hiện tại: ___")

        if KH_SSO_ENABLED:
            import gradiologin as grlogin

            self.sso_signout = grlogin.LogoutButton("Đăng xuất")
        else:
            self.signout = gr.Button("Đăng xuất")

            self.password_change = gr.Textbox(
                label="Mật khẩu mới", interactive=True, type="password"
            )
            self.password_change_confirm = gr.Textbox(
                label="Xác nhận mật khẩu", interactive=True, type="password"
            )
            self.password_change_btn = gr.Button("Thay đổi mật khẩu", interactive=True)
    ...
    def change_password(self, user_id, password, password_confirm):
        ...
        with Session(engine) as session:
            ...
            if result:
                ...
                session.add(user)
                session.commit()
                gr.Info("Mật khẩu đã được thay đổi")
            else:
                gr.Warning("Không tìm thấy người dùng")

        return "", ""

    def app_tab(self):
        with gr.Tab("Tổng quan", visible=self._render_app_tab):
    ...
    def index_tab(self):
        ...
        with gr.Tab("Cài đặt truy xuất", visible=self._render_index_tab):
    ...
    def reasoning_tab(self):
        with gr.Tab("Cài đặt lý luận", visible=self._render_reasoning_tab):
            with gr.Group():
            ...
            gr.Markdown("### Cài đặt riêng cho từng loại lý luận")
    ...
    def save_setting(self, user_id: int, *args):
        ...
        if user_id is None:
            gr.Warning("Cần đăng nhập trước khi lưu cài đặt")
            return setting
    ...
    def _on_app_created(self):
        ...
        def update_llms():
            ...
            if llms._default:
                llm_choices = [(f"{llms._default} (mặc định)", "")]
            else:
                llm_choices = [("(ngẫu nhiên)", "")]
        ...
        def update_embeddings():
            ...
            if embedding_models_manager._default:
                emb_choices = [(f"{embedding_models_manager._default} (mặc định)", "")]
            else:
                emb_choices = [("(ngẫu nhiên)", "")]
