...
def is_conv_name_valid(name):
    ...
    if len(name) == 0:
        ...
        errors.append("Tên không được dài quá 40 ký tự")
...
class ConversationControl(BasePage):
    ...
    def on_building_ui(self):
        with gr.Row():
            title_text = "Cuộc trò chuyện" if not KH_DEMO_MODE else "Bài báo Kotaemon"
        ...
        with gr.Row() as self._new_delete:
            ...
            self.cb_is_public = gr.Checkbox(
                value=False,
                label="Chia sẻ cuộc trò chuyện này",
        ...
        if KH_DEMO_MODE:
            with gr.Row():
                self.btn_demo_login = gr.Button(
                    "Đăng nhập để tạo trò chuyện mới",
                    min_width=120,
                    size="sm",
                    scale=1,
                    variant="primary",
        ...
        with gr.Row(visible=False) as self._delete_confirm:
            ...
            self.btn_del_cnl = gr.Button(value="Hủy", min_width=10)
    ...
    def new_conv(self, user_id):
        """Create new chat"""
        if user_id is None:
            gr.Warning("Vui lòng đăng nhập trước (Cài đặt → Cài đặt Người dùng)")
        ...
        with Session(engine) as session:
            ...
            try:
                ...
                info_panel = (
                    ...
                    else "<h5><b>Không tìm thấy bằng chứng.</b></h5>"
                )
    ...
    def rename_conv(self, conversation_id, new_name, is_renamed, user_id):
        ...
        history = self.load_chat_history(user_id)
        gr.Info("Đã đổi tên cuộc trò chuyện.")
        return (
    ...
    def persist_chat_suggestions(
        ...
        gr.Info("Đã cập nhật gợi ý trò chuyện.")
