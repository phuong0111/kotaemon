...
DEFAULT_SETTING = "(mặc định)"
INFO_PANEL_SCALES = {True: 8, False: 4}
DEFAULT_QUESTION = (
    "Tóm tắt của tài liệu này là gì?"
    if not KH_DEMO_MODE
    else "Tóm tắt của bài báo này là gì?"
)
...
class ChatPage(BasePage):
    ...
    def on_building_ui(self):
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_id="conv-settings-panel") as self.conv_column:
                ...
                for index_id, index in enumerate(self._app.index_manager.indices):
                    ...
                    if KH_DEMO_MODE and is_first_index:
                        index_name = "Chọn từ Bộ sưu tập Bài báo"
                ...
                if len(self._app.index_manager.indices) > 0:
                    quick_upload_label = (
                        "Tải lên Nhanh" if not KH_DEMO_MODE else "Hoặc nhập URL bài báo mới"
                    )
                ...
                if not KH_DEMO_MODE:
                    ...
                    with gr.Accordion(label="Bài báo liên quan", open=False):
            ...
            with gr.Column(
                ...
                with gr.Accordion(
                    label="Bảng thông tin", open=True, elem_id="info-expand"
                ):
    ...
    def on_register_events(self):
        ...
        self.use_mindmap_check.change(
            lambda x: (x, gr.update(label="Sơ đồ tư duy " + ("(bật)" if x else "(tắt)"))),
        ...
        def raise_error_on_state(state):
            if not state:
                raise ValueError("Gợi ý trò chuyện đã tắt")
    ...
    def submit_msg(
        ...
        if not chat_input:
            raise ValueError("Đầu vào trống")
        ...
        if chat_input_text:
            ...
            if not chat_history:
                raise gr.Error("Cuộc trò chuyện trống")
    ...
    def on_set_public_conversation(self, is_public, convo_id):
        if not convo_id:
            gr.Warning("Chưa chọn cuộc trò chuyện nào")
            return
        ...
        with Session(engine) as session:
            ...
            if result.is_public != is_public:
                ...
                gr.Info(
                    f"Cuộc trò chuyện: {name} đã {'công khai' if is_public else 'riêng tư'}."
                )
    ...
    def reasoning_changed(self, reasoning_type):
        if reasoning_type != DEFAULT_SETTING:
            ...
            gr.Info("Loại lý luận đã thay đổi thành `{}`".format(reasoning_type))
    ...
    def create_pipeline(
        ...
        *selecteds,
        ...
        if command_state == WEB_SEARCH_COMMAND:
            ...
            if not WebSearch:
                raise ValueError("Backend tìm kiếm web không khả dụng.")
    ...
    def chat_fn(
        ...
        *selecteds,
        ...
        msg_placeholder = getattr(
            flowsettings, "KH_CHAT_MSG_PLACEHOLDER", "Đang suy nghĩ ..."
        )
        ...
        if not text:
            empty_msg = getattr(
                flowsettings, "KH_CHAT_EMPTY_MSG_PLACEHOLDER", "(Xin lỗi, tôi không biết)"
            )
