...
class LLMManagement(BasePage):
    def __init__(self, app):
        ...
        self.spec_desc_default = (
            "# Mô tả đặc tả\n\nChọn một LLM để xem mô tả đặc tả."
        )
        self.on_building_ui()
                ...
                with gr.Row():
                    with gr.Column():
                        self.edit_default = gr.Checkbox(
                            ...
                            info=(
                                "Đặt LLM này làm mặc định. Nếu không có mặc định nào được đặt, "
                                "một LLM ngẫu nhiên sẽ được sử dụng."
                            ),
                        )
                        self.edit_spec = gr.Textbox(
                            label="Đặc tả",
                            info="Đặc tả của LLM ở định dạng YAML",
                            lines=10,
                        )
                        ...
                        with gr.Accordion(
                            ...
                            with gr.Row():
                                with gr.Column(scale=4):
                                    self.connection_logs = gr.HTML("Nhật ký")
            ...
            with gr.Row():
                with gr.Column(scale=2):
                    self.name = gr.Textbox(
                        label="Tên LLM",
                        info=(
                            "Phải là duy nhất. Tên sẽ được sử dụng để xác định LLM."
                        ),
                    )
                    self.llm_choices = gr.Dropdown(
                        label="Nhà cung cấp LLM",
                        info=(
                            "Chọn nhà cung cấp cho LLM. Mỗi nhà cung cấp có "
                            "đặc tả khác nhau."
                        ),
                    )
                    ...
                    self.default = gr.Checkbox(
                        label="Đặt làm mặc định",
                        info=(
                            "Đặt LLM này làm mặc định. LLM mặc định này sẽ được sử dụng "
                            "theo mặc định trong toàn bộ ứng dụng."
                        ),
                    )
                    self.btn_new = gr.Button("Thêm LLM", variant="primary")
    ...
    def create_llm(self, name, choices, spec, default):
        try:
            ...
            gr.Info(f"LLM {name} được tạo thành công")
        except Exception as e:
            raise gr.Error(f"Không thể tạo LLM {name}: {e}")
    ...
    def select_llm(self, llm_list, ev: gr.SelectData):
        if ev.value == "-" and ev.index[0] == 0:
            gr.Info("Chưa có LLM nào được tải. Vui lòng thêm LLM trước")
            return ""

        if not ev.selected:
            return ""

        return llm_list["tên"][ev.index[0]]
    ...
    def check_connection(self, selected_llm_name: str, selected_spec):
        ...
        try:
            log_content += f"- Đang kiểm tra mô hình: {selected_llm_name}<br>"
            yield log_content
            ...
            if llm is None:
                raise Exception(f"Không thể tìm thấy mô hình: {selected_llm_name}")
            ...
            log_content += (
                f"<mark style='background: green; color: white'>- Kết nối thành công. "
                f"Nhận được phản hồi:\n {respond}</mark><br>"
            )
            yield log_content

            gr.Info(f"LLM {selected_llm_name} kết nối thành công")
        except Exception as e:
            log_content += (
    ...
    def save_llm(self, selected_llm_name, default, spec):
        try:
            ...
            gr.Info(f"LLM {selected_llm_name} đã được lưu thành công")
        except Exception as e:
            raise gr.Error(f"Không thể lưu LLM {selected_llm_name}: {e}")

    def delete_llm(self, selected_llm_name):
        try:
            llms.delete(selected_llm_name)
        except Exception as e:
            gr.Error(f"Không thể xóa LLM {selected_llm_name}: {e}")
