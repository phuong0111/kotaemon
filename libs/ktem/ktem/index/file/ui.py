...
DOWNLOAD_MESSAGE = "Bắt đầu tải xuống"
...
class DirectoryUpload(BasePage):
    ...
    def on_building_ui(self):
        with gr.Accordion(label="Tải lên thư mục", open=False):
            gr.Markdown(f"Các loại tệp được hỗ trợ: {self._supported_file_types_str}")
            self.path = gr.Textbox(
                placeholder="Đường dẫn thư mục...", lines=1, max_lines=1, container=False
            )
            with gr.Accordion("Tùy chọn chỉ mục nâng cao", open=False):
                with gr.Row():
                    self.reindex = gr.Checkbox(
                        value=False, label="Buộc lập chỉ mục lại tệp", container=False
                    )

            self.upload_button = gr.Button("Tải lên và Chỉ mục")
...
class FileIndexPage(BasePage):
    def __init__(self, app, index):
        ...
        self.selected_panel_false = "Tệp đã chọn: (vui lòng chọn ở trên)"
        self.selected_panel_true = "Tệp đã chọn: {name}"
    ...
    def upload_instruction(self) -> str:
        ...
        if self._supported_file_types:
            msgs.append(f"- Các loại tệp được hỗ trợ: {self._supported_file_types_str}")

        if max_file_size := self._index.config.get("max_file_size", 0):
            msgs.append(f"- Kích thước tệp tối đa: {max_file_size} MB")

        if max_number_of_files := self._index.config.get("max_number_of_files", 0):
            msgs.append(f"- Chỉ mục có thể có tối đa {max_number_of_files} tệp")
    ...
    def render_file_list(self):
        self.filter = gr.Textbox(
            ...
            info=(
                ...
                "(2) Tìm kiếm với chuỗi rỗng để hiển thị tất cả tệp."
            ),
        )
        ...
        self.file_list = gr.DataFrame(
            ...
            column_widths=["0%", "40%", "15%", "12%", "18%", "15%"],
            interactive=False,
            wrap=False,
        ...
        with gr.Accordion("Tùy chọn nâng cao", open=False):
            with gr.Row():
                ...
                self.delete_all_button_confirm = gr.Button(
                    "Xác nhận xóa", variant="stop", visible=False
                )
                self.delete_all_button_cancel = gr.Button("Hủy", visible=False)
        ...
        with gr.Row():
            with gr.Column(scale=1):
                with gr.Column() as self.upload:
                    ...
                    with gr.Tab("Sử dụng Liên kết Web"):
                        self.urls = gr.Textbox(
                            label="Nhập URL web",
                            lines=8,
                        )
                        gr.Markdown("(Xuống dòng để nhập url mới)")
            ...
            with gr.Column(scale=4):
                with gr.Column(visible=False) as self.upload_progress_panel:
                    gr.Markdown("## Tiến trình Tải lên")
                    with gr.Row():
                        self.upload_result = gr.Textbox(
                            lines=1, max_lines=20, label="Kết quả tải lên"
                        )
                        self.upload_info = gr.Textbox(
                            lines=1, max_lines=20, label="Thông tin tải lên"
                        )
                    self.btn_close_upload_progress_panel = gr.Button(
                        "Xóa Thông tin Tải lên và Đóng",
                        variant="secondary",
                        elem_classes=["right-button"],
                    )

                # Groups section (above)
                with gr.Column():
                    gr.Markdown("## Nhóm")
            ...
            with Session(engine) as session:
                ...
                for idx, doc in enumerate(docs):
                    ...
                    if doc.metadata.get("page_label"):
                        header_prefix += f" [Trang {doc.metadata['page_label']}]"
    ...
    def download_single_file(self, is_zipped_state, file_id):
        ...
        if is_zipped_state:
            new_button = gr.DownloadButton(label="Tải xuống", value=None)
        else:
    ...
    def download_all_files(self):
        if self._index.config.get("private", False):
            raise gr.Error("Tính năng này không có sẵn cho bộ sưu tập riêng tư.")
    ...
    def show_delete_all_confirm(self, file_list):
        ...
        if len(file_list) == 0 or (
            ...
            gr.Info("Không có tệp nào để xóa")
            return [
                gr.update(visible=True),
                gr.update(visible=False),
                gr.update(visible=False),
                ...
                if not KH_DEMO_MODE:
                    quickUploadedEvent = (
                        self._app.chat_page.quick_file_upload.upload(
                            fn=lambda: gr.update(
                                value="Vui lòng chờ quá trình chỉ mục hoàn thành "
                                "trước khi thêm câu hỏi của bạn."
                            ),
                    ...
                    quickUploadedEvent = (
                        ...
                        .then(
                            fn=lambda: gr.update(value="Chỉ mục hoàn thành."),
                            outputs=self._app.chat_page.quick_file_upload_status,
                        )
                        .then(
                            fn=self.list_file,
    ...
    def index_fn(
        ...
        gr.Info(f"Bắt đầu chỉ mục {len(files)} tệp...")
        ...
        try:
            ...
            debugs.append(f"Lỗi: {e}")
            yield "\n".join(outputs), "\n".join(debugs)
            return

        n_successes = len([_ for _ in results if _])
        if n_successes:
            gr.Info(f"Chỉ mục thành công {n_successes} tệp")
    ...
    def index_fn_url_with_default_loaders(
        ...
        if KH_DEMO_MODE:
            ...
            if not all(is_arxiv_url(url) for url in urls_splitted):
                raise ValueError("Tất cả URL phải là URL arXiv hợp lệ")
    ...
    def index_files_from_dir(
        ...
        if include_patterns and exclude_patterns:
            raise ValueError("Không thể có cả mẫu bao gồm và loại trừ")
            ...
            results = [
                {
                    "id": each[0].id,
                    "tên": each[0].name,
                    "kích thước": self.format_size_human_readable(each[0].size),
                    "token": self.format_size_human_readable(
                        each[0].note.get("tokens", "-"), suffix=""
                    ),
                    "trình tải": each[0].note.get("loader", "-"),
                    "ngày tạo": each[0].date_created.strftime("%Y-%m-%d %H:%M:%S"),
                }
    ...
    def list_file_names(self, file_list_state):
        if file_list_state:
            file_names = [(item["tên"], item["id"]) for item in file_list_state]
        else:
            file_names = []
    ...
    def list_group(self, user_id, file_list):
        ...
        if file_list:
            file_id_to_name = {item["id"]: item["tên"] for item in file_list}
        else:
            file_id_to_name = {}
        ...
        if results:
            ...
            for item in formated_results:
                file_names = [
                    file_id_to_name.get(file_id, "-") for file_id in item["tệp"]
                ]
                item["tệp"] = ", ".join(
                ...
                item["tệp"] = f"[{item_count} mục{item_postfix}] " + item["tệp"]
    ...
    def delete_group(self, group_id):
        if not group_id:
            raise gr.Error("Không có nhóm nào được chọn")
        ...
        with Session(engine) as session:
            ...
            if group:
                ...
                session.delete(item)
                session.commit()
                gr.Info(f"Nhóm {group_name} đã được xóa")
            else:
                raise gr.Error("Không tìm thấy nhóm")

        return None

    def interact_file_list(self, list_files, ev: gr.SelectData):
        if ev.value == "-" and ev.index[0] == 0:
            gr.Info("Không có tệp nào được tải lên")
            return None, self.selected_panel_false
        ...
        return list_files["id"][ev.index[0]], self.selected_panel_true.format(
            name=list_files["tên"][ev.index[0]]
        )
    ...
    def validate_files(self, files: list[str]):
        ...
        if max_file_size := self._index.config.get("max_file_size", 0):
            ...
            if errors_max_size:
                ...
                errors.append(
                    f"Kích thước tệp tối đa ({max_file_size} MB) bị vượt quá: {str_errors}"
                )
        ...
        if max_number_of_files := self._index.config.get("max_number_of_files", 0):
            ...
            if len(paths) + current_num_files > max_number_of_files:
                errors.append(
                    f"Số lượng tệp tối đa ({max_number_of_files}) sẽ bị vượt quá"
                )
    ...
    def validate_urls(self, urls: list[str]):
        ...
        for url in urls:
            if not url.startswith("http") and not url.startswith("https"):
                errors.append(f"URL không hợp lệ `{url}`")
        return errors
...
class FileSelector(BasePage):
    ...
    def on_register_events(self):
        def update_selector_visibility(mode, user_id, selected_files):
            # Update visibility and reload options based on mode
            if mode in ["files", "groups"]:
                # Reload options with the new mode
                updated_selector, updated_choices = self.load_files(selected_files, user_id, mode)
                return (
                    gr.update(visible=True, **updated_selector), 
                    user_id,
                    updated_choices
                )
            else:  # "all" or "disabled"
                return (
                    gr.update(visible=False), 
                    user_id,
                    []
        ...
        self.mode.change(
            ...
            inputs=[self.mode, self._app.user_id, self.selector],
            outputs=[self.selector, self.selector_user_id, self.selector_choices],
        )
    ...
    def get_selected_ids(self, components):
        ...
        if mode == "disabled":
            ...
            # Filter out group selections (those that start with JSON format)
            file_ids = []
            if selected:
                for item in selected:
                    try:
                        # If it's a JSON string, it's a group - skip it
                        json.loads(item)
                    except (json.JSONDecodeError, TypeError):
                        # It's a regular file ID
                        file_ids.append(item)
            return file_ids
        elif mode == "groups":
            # Convert group selections to file IDs
            file_ids = []
            if selected:
                for item in selected:
                    try:
                        # Group data should be JSON string of file IDs
    ...
    def load_files(self, selected_files, user_id, mode=None):
        options: list = []
        available_ids = []
        ...
        with Session(engine) as session:
            ...
            file_results = session.execute(statement).all()
            ...
            if self._index.config.get("private", False):
                statement = statement.where(FileGroup.user == user_id)
            group_results = session.execute(statement).all()

        # Add files or groups based on mode
        if mode == "files":
            # Only show individual files
            ...
            for result in group_results:
                item = result[0]
                options.append(
                    (f"nhóm: '{item.name}'", json.dumps(item.data.get("files", [])))
                )
        else:
            # Default behavior for backward compatibility - show both
            for result in file_results:
                available_ids.append(result[0].id)
                options.append((result[0].name, result[0].id))
        ...
        # Clean up selected files based on available options
        if selected_files and mode == "files":
            available_ids_set = set(available_ids)
            selected_files = [
                each for each in selected_files if each in available_ids_set
            ]
        elif mode == "groups":
            # Clear file selections when switching to groups mode
            selected_files = []
