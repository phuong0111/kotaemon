...
def clean_mlx_response(content: str) -> str:
    """Clean MLX response by extracting final message content"""
    if not content:
        return content
    ...
    # If no final channel found, remove all channel tags
    cleaned = re.sub(r'<\|[^>]+\|>[^<]*?(?=<\||\Z)', '', content, flags=re.DOTALL)
